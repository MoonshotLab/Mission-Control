...
exports.events = function(id){
  ...
  return new Promise(function(resolve, reject){
    db.getRecordById('events', id).then(function(record){
      ...
      var textContent = [
        '<tvce-attributedText style="' + styles + 'width:1538; margin:102 0 0 191; font-size:49;">ROCKETU</tvce-attributedText>',
        '<tvce-attributedText style="' + styles + 'width:1538; tvcs-text-decoration:text-shadow; margin:90 0 0 191; font-size:110;">' + record.summary.toUpperCase() + '</tvce-attributedText>',
        '<tvce-attributedText style="' + styles + 'width:1538; margin:90 0 0 200; font-size:61;">' + details.join('\n').toUpperCase() + '</tvce-attributedText>',
      ];
