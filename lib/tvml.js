...
exports.gratitude = function(opts){
  var textContent = [
    '<tvce-attributedText style="' + styles + 'margin:102 0 0 194; font-size:49;">GRATITUDES</tvce-attributedText>',
    '<row style="tv-position:top-left; height:781;"><tvce-attributedText style="' + styles + 'width:1538; margin:90 0 0 191; font-size:89;">' + opts.body.toUpperCase() + '</tvce-attributedText></row>',
    '<tvce-attributedText style="' + styles + 'margin:0 0 0 1400; font-size:49;">' + opts.from.name.toUpperCase() + '</tvce-attributedText>',
  ].join('');

  return createDivDocument(textContent);
};



exports.event = function(opts){
  var date        = moment(opts.start.dateTime).format('dddd, MMMM Do');
  var startTime   = moment(opts.start.dateTime).format('h:mm');
  var endTime     = moment(opts.end.dateTime).format('h:mm a');
  var description = [
    date,
    startTime + '-' + endTime + '|' + opts.location
  ];

  if(opts.rsvpCode){
    description.push([
      'RSVP: Text', opts.rsvpCode, 'to', config.PHONE_NUMBER
    ].join(' '));
  }

  var textContent = [
    '<tvce-attributedText style="' + styles + 'margin:102 0 0 191; font-size:49;">ROCKETU</tvce-attributedText>',
    '<tvce-attributedText style="' + styles + 'tvcs-text-decoration:text-shadow; margin:90 0 0 191; font-size:110;">' + opts.summary.toUpperCase() + '</tvce-attributedText>',
