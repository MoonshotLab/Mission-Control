...
exports.newGratitude = function(req, res){
  ...
  req.on('end', function(){
    ...
    utils.log(2, 'new gratitude received', gratitude);
  ...
  db.getEvents({ rsvpcode : userInput }).then(function(events){
    ...
    db.getUsers({ phoneNumber : phoneNumber }).then(function(users){
      if(events && users){
        ...
        var message = [
          'Thanks! You\'ve RSVP\'d to',
          event.summary + '.',
          'It\'s been added to your calendar and a reminder e-mail',
          'is on the way.'
        ].join(' ');
        var textMessage = utils.createTextMessage(message);
        res.send(textMessage);
      }
    }).catch(utils.logError);
  }).catch(function(err){
    var textMessage = utils.createTextMessage(err);
    res.send(textMessage);
    utils.logError(err);
  });
};
