...
exports.newEmail = function(req, res){
  ...
  req.on('end', function(){
    ...
    // we're using a distribution list to parse messages, so ensure the
    // intended recipient is gratitudes
    var toAddress = email.message_data.addresses.to.email;
    if(toAddress == 'gratitudes@barkleyus.com'){
