...
exports.birthdays = function(req, res){
  var now   = new Date();
  var month = now.getMonth();
  var day   = now.getDate();
  var query = { 'birthday.month' : month, 'birthday.day' : day };
...
exports.anniversaries = function(req, res){
  var now   = new Date();
  var month = now.getMonth();
  var day   = now.getDate();
  var query = { 'anniversary.month' : month, 'anniversary.day' : day };
...
// External Services
exports.gratitudeError = function(req, res){
  utils.log(1, 'email error', req.body);
  res.sendStatus(200);
};
  ...
  if(email.message_data.references.length &&
    ...
    if(charCount > 140 && config.MODE == 'production'){
      ...
      google.sendMail({
        ...
        body      : 'Your gratitude is great, but it\'s just a little too long. Please stay brief and keep it under <b>140 characters</b>. \n If you\'re attempting to submit more than one gratitude please send them one at a time. \n\n You can reply to this e-mail.'
      });
