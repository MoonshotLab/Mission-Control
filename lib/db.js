...
var getEvents = function(query, limit){
  ...
  var maxDate = new Date(new Date().getTime() + 2592000000);
  if(query.future){
    query['start.dateTime'] = { $gte : new Date() };
    query['end.dateTime']   = { $lte : maxDate    };
    delete query.future;
  }
  ...
  return new Promise(function(resolve, reject){
    getEvents(query, limit).then(function(events){
      if(!events.length) resolve(null);
        ...
        events.sort(compare);
        if(events.length) resolve(events[0]);
        else resolve(null);
      }
    }).catch(reject);
  });
...
exports.deleteGratitude = function(messageIds){
  utils.log(2, 'deleting gratitude', messageIds);
